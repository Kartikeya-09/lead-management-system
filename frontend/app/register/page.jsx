  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (response?.status === 409) {
        setServerError(response.data?.error?.message || 'Email already exists');
        return;
      }
      ...
      setServerError(response?.data?.error?.message || 'Unable to create your account right now.');
    }
  };
        ...
        <div className="grid w-full overflow-hidden rounded-4xl border border-white/10 bg-white/5 shadow-2xl shadow-slate-950/40 backdrop-blur-xl lg:grid-cols-[0.95fr_1.05fr]">
          <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.18),transparent_40%),linear-gradient(135deg,rgba(15,23,42,0.98),rgba(30,41,59,0.96))] p-8 lg:p-10">
            ...
            <h1 className="max-w-md text-4xl font-semibold leading-tight tracking-tight md:text-5xl">
              Join a cleaner, calmer way to manage sales.
            </h1>
            <p className="mt-5 max-w-md text-base leading-7 text-slate-300">
              Sign up to start capturing leads, assigning owners, and tracking the full deal journey in one place.
            </p>

            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              {[
                ['Fast setup', 'Create an account in under a minute.'],
                ['Private by default', 'Access is role-aware on the backend.'],
                ['Ready to scale', 'Built for teams that move quickly.'],
              ].map(([title, text]) => (
          ...
          <section className="bg-slate-50 p-8 text-slate-900 lg:p-10">
            <div className="mx-auto max-w-xl">
              <div className="mb-8">
                ...
                <h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Create your workspace account</h2>
                <p className="mt-3 text-sm leading-6 text-slate-600">This will create a member account and log you in right away.</p>
              </div>
